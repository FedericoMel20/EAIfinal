...
const uniqueName = (value, instructorId) => { const name = clean(value, 'Name'); if (instructors.some((instructor) => instructor.id !== instructorId && instructor.name.toLowerCase() === name.toLowerCase())) throw new GraphQLError('An instructor with this name already exists.', { extensions: { code: 'DUPLICATE_RECORD' } }); return name; };

module.exports = {
  instructors: () => instructors.map(view),
  instructor: ({ id }) => { const item = get(id); if (!item) throw missing(id); return view(item); },
  createInstructor: ({ input }) => { const item = { id: nextId(), name: uniqueName(input.name), department: clean(input.department, 'Department') }; instructors.push(item); return view(item); },
  updateInstructor: ({ id, input }) => { const item = get(id); if (!item) throw missing(id); if (input.name !== undefined) item.name = uniqueName(input.name, item.id); if (input.department !== undefined) item.department = clean(input.department, 'Department'); return view(item); },
