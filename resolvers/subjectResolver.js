...
module.exports = {
  ...
  updateSubject: ({ id, input }) => {
    ...
    if (input.name !== undefined) {
      ...
      if (subjects.some((subject) => subject.id !== item.id && subject.name.toLowerCase() === name.toLowerCase())) throw new GraphQLError('A subject with this name already exists.', { extensions: { code: 'DUPLICATE_RECORD' } });
      item.name = name;
    }
    return view(item);
  },
  deleteSubject: ({ id }) => {
    const index = subjects.findIndex((s) => s.id === String(id)); if (index < 0) throw missing(id);
    for (let scoreIndex = scores.length - 1; scoreIndex >= 0; scoreIndex -= 1) if (scores[scoreIndex].subjectId === String(id)) scores.splice(scoreIndex, 1);
