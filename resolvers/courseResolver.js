...
const credits = (value) => { if (!Number.isInteger(value) || value <= 0) fail('Credits must be greater than zero.'); return value; };
...
module.exports = {
  ...
  createCourse: ({ input }) => { const course = { id: nextId(), title: title(input.title), credits: credits(input.credits), instructorId: instructorId(input.instructorId) }; courses.push(course); return view(course); },
  updateCourse: ({ id, input }) => { const course = get(id); if (!course) throw missing(id); if (input.title !== undefined) course.title = title(input.title); if (input.credits !== undefined) course.credits = credits(input.credits); if (input.instructorId !== undefined) course.instructorId = instructorId(input.instructorId); return view(course); },
